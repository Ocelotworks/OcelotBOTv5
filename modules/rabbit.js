...
module.exports = {
    ...
    init: async function(bot) {
        try {
            ...
            bot.rabbit.eventsChannel.assertQueue(eventsQueue, {durable: false});
            ...
            bot.bus.on("modulesLoaded", ()=>{
                bot.client.on("ready", function () {
                    bot.rabbit.rpcChannel.assertQueue(`reply-${bot.client.user.id}-${bot.util.shard}`, {durable: false});
            ...
            bot.rabbit.rpc = async function (name, payload, timeout = 300000, config) {
                return new Promise(function (fulfill) {
                    ...
                    bot.rabbit.rpcChannel.sendToQueue(name, Buffer.from(JSON.stringify(payload)), {
                        correlationId,
                        replyTo: `reply-${bot.client.user.id}-${bot.util.shard}`
                    });
            ...
            console.error("Unable to initialise rabbit: ", e);
            if(!bot.client){
                if(retries++ > 5){
                    console.error("Retried 5 times, exiting.");
                    process.exit(1);
                }
                bot.logger.log("Client is not connected, retrying rabbit in 5s...");
                await new Promise((fulfill)=>{
                    setTimeout(async ()=>{
                        bot.logger.log("Retrying...")
                        fulfill(await module.exports.init(bot))
                    }, 5000);
                })
            }
        }
