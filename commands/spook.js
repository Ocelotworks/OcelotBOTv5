    ...
    init: async function (bot) {
        ...
        bot.spook.checkSpecialRoles = async function checkSpecialRoles(channel, spooker, spooked) {
            ...
            if (!spookerUser || !spookedUser || spookerUser.user.bot || spookedUser.user.bot) {
                ...
                let targets = channel.guild.members.cache.filter(function (member) {
                    return !member.user.bot && member.presence.status !== "offline" && member.id !== spooker.id
                });
                ...
                embed.addField(role.name.toUpperCase(), role.desc.formatUnicorn({
                    spooked: target,
                    spooker,
                    num: required
                }));
        ...
        bot.spook.generateNew = async function generateNew(server) {
            ...
            await channel.sendLang(left ? "SPOOK_USER_LEFT" : "SPOOK_USER_IDLE", {
                old: lastSpooked,
                spooked: targetUser.id
            });
        };
